...
for i = 1:m_ForecastData
    if ForecastData(i,5) == 0 
        new_version_ResultingData(i,12) = Result_cluster_final(j,48);
    else
        new_version_ResultingData(i,12) = Result_cluster_final(j,(ForecastData(i,5)*2));
    end
    if i == k && i~=1
        j = j + 1;
    end
end
predictedPVGen = new_version_ResultingData(1:m_ForecastData,12);
