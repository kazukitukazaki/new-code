...
elseif length(mv)==D     
 velmaskmin = repmat(PVset_forcerow(-mv),ps,1); % min vel
 velmaskmax = repmat(PVset_forcerow( mv),ps,1); % max vel
else
...
% format of pos= (particle#, dimension)
 % construct random population positions bounded by VR
  pos(1:ps,1:D) = PVset_normmat(rand([ps,D]),VR',1);
 ...
 % construct initial random velocities between -mv,mv
  vel(1:ps,1:D) = PVset_normmat(rand([ps,D]),...
      [PVset_forcecol(-mv),PVset_forcecol(mv)]',1);
